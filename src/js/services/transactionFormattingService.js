(function() {
  ...
  function TransactionFormattingService(
    $http,
    $q,
    apiService,
    cryptoService,
    ...
    processors[
      constants.PAYMENT_TRANSACTION_TYPE
    ] = postProcessPaymentTransaction;
    processors[
      constants.ASSET_ISSUE_TRANSACTION_TYPE
    ] = postProcessIssueTransaction;
    processors[
      constants.ASSET_TRANSFER_TRANSACTION_TYPE
    ] = postProcessTransferTransaction;
    processors[
      constants.ASSET_BURN_TRANSACTION_TYPE
    ] = postProcessBurnTransaction;
    processors[
      constants.EXCHANGE_TRANSACTION_TYPE
    ] = postProcessExchangeTransaction;
    processors[
      constants.ASSET_REISSUE_TRANSACTION_TYPE
    ] = postProcessReissueTransaction;
    processors[
      constants.START_LEASING_TRANSACTION_TYPE
    ] = postProcessLeasingTransaction;
    processors[
      constants.MASS_PAYMENT_TRANSACTION_TYPE
    ] = postProcessMassPaymentTransaction;
    ...
    function postProcessIssueTransaction(transaction) {
      ...
      transaction.extras.amount.amount = Money.fromCoins(
    ...
    function postProcessExchangeTransaction(transaction) {
      ...
      if (pair.amountAsset === null) pair.amountAsset = "";
      if (pair.priceAsset === null) pair.priceAsset = "";
      ...
      var from =
        transaction.order1.orderType === "buy"
          ? transaction.order2.senderPublicKey
          : transaction.order1.senderPublicKey;
      var to =
        transaction.order1.orderType === "sell"
          ? transaction.order2.senderPublicKey
          : transaction.order1.senderPublicKey;

      var price;
      if (
        Currency.isCached(pair.amountAsset) &&
        Currency.isCached(pair.priceAsset)
      ) {
        var currencyPair = {
          amountAsset: Currency.create({ id: pair.amountAsset }),
          priceAsset: Currency.create({ id: pair.priceAsset })
        };
        var orderPrice = OrderPrice.fromBackendPrice(
          transaction.price,
          currencyPair
        ).toTokens();
        var amount = Money.fromCoins(
          transaction.amount,
          currencyPair.amountAsset
        );
        ...
        transaction.extras.total.amount = (price * amount.toTokens()).toFixed(
          currencyPair.priceAsset.precision
        );
    ...
    function formatFee(fee, currency) {
      return (
        Money.fromCoins(fee, currency).formatAmount(true) +
        " " +
        currency.toString()
      );
    }
    ...
    function processTransaction(transaction) {
      ...
      if (processor) processor(transaction);
      else postProcessAmountlessTransaction(transaction);
    }

    function updateFee(transaction) {
      if (!transaction.feeAsset) return;
    ...
    function updateExchangeCurrencies(transaction) {
      ...
      if (
        ...
        var price = OrderPrice.fromBackendPrice(
          transaction.price,
          currencyPair
        ).toTokens();
        var amount = Money.fromCoins(
        ...
        transaction.extras.price = OrderPrice.fromBackendPrice(
          transaction.price,
          currencyPair
        )
          .toTokens()
    ...
    this.processAmountAndFee = function(transactions) {
      ...
      var promises = Object.keys(loadAssets).map(function(assetId) {
        return $http
          .get(apiService.transactions.info(assetId))
  ...
  angular
    .module("web")
    .service("transactionFormattingService", [
      "$http",
      "$q",
      "apiService",
      "cryptoService",
